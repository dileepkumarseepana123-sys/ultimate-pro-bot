import json, re, math, zipfile, sys
...
SESSION.headers.update({"User-Agent": "Ultimate-Pro-Terminal/10.0"})

# VIP Franchise Leagues & Major International Teams
PREMIUM_KEYWORDS = ["cpl", "caribbean", "ipl", "indian premier", "bbl", "big bash", "psl", "pakistan super", "sa20", "blast", "t20i", "twenty20"]
MAJOR_TEAMS = ["england", "india", "australia", "sri lanka", "west indies", "south africa", "new zealand", "pakistan", "bangladesh", "afghanistan"]
JUNK_WORDS = ["club", "university", "college", "academy", "regional", "minor", "county", "local", "u19", "under-19", "women's club"]
...
def ensure_cricsheet_history():
    ...
    print("[INFO] Downloading Cricsheet History (150MB)... This happens only once.")
    ...
    for p in HISTORY_DIR.glob("*.json"):
        try:
            with p.open("r", encoding="utf-8") as f:
                data = json.load(f)
                info = data.get("info", {})
                m_type = str(info.get("match_type", "")).lower()
                # Strict T20 Filter
...
def analyze_venue(history, target_venue):
    ...
    for m in history:
        ...
        # Spin Squeeze Index (Overs 6-13, which is 7th to 14th over)
        for inn in innings:
    ...
    if matches_found < 5: return None # Insufficient Data Honesty Protocol
...
# ==========================================
# ESPN HYBRID DATA FUSION ENGINE
# ==========================================
def run_scanner():
    print("=" * 60)
    print("ULTIMATE PRO TERMINAL V10 (Cricsheet + ESPN + Open-Meteo)")
    print("=" * 60)
    
    # Init Cricsheet History
    ...
    try:
        ...
        for event in data.get('events', []):
            ...
            # RULE 1: STRICT T20 FILTER
            if "hundred" in combined_text: continue # The Hundred is 100-balls, reject.
            if not ("t20" in combined_text or "twenty20" in combined_text): continue
            if any(j in combined_text for j in JUNK_WORDS): continue
            
            # RULE 2: PREMIUM vs JUNK CLASSIFICATION
            is_franchise = any(k in combined_text for k in PREMIUM_KEYWORDS)
            is_intl = ("t20i" in combined_text) and any(t in combined_text for t in MAJOR_TEAMS)
            ...
            # RULE 3 & 4: CRICSHEET + METEO FUSION
            if is_premium:
    ...
    print(f"Data Fusion Complete. Saved {len(live_matches)} matches.")
